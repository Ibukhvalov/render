...
impl BVH {
    pub fn init_from_vec(mut hittable_list: Vec<HittableSurfaces>, deep: u32) -> Self {
        ...
        let mut axis_ranges: Vec<(usize, f32)> = (0..3).map(|axis| {
            (axis, util::axis_range(&hittable_list, axis))
        }).collect();
        ...
        match len {
            0 => { panic!["empty initial vector!"]},
            1 => {
                let surface = hittable_list.pop().unwrap();
                if let Some(bbox) = surface.get_bbox() {
                    BVH {node: Node::Leaf(Box::new(surface)), bbox , deep}
                } else {
            ...
            _ => {
                ...
                BVH { node: Node::Branch {left: Box::new(left), right: Box::new(right)}, bbox, deep}
